...
class RabbitViewer():
    ...
    def render(self, state, t, ref, u, r):
        ...
        pr, p0, pk, p1, p2 = mp.node_pos(state)
        ...
        pygame.draw.line(self.screen, GRAY, pro, p0o, width=int(300 * RSCALE))
        pygame.draw.line(self.screen, GRAY, pro, p1o, width=int(100 * RSCALE))
        pygame.draw.line(self.screen, BLACK, p0o, pko, width=int(100 * RSCALE))
        pygame.draw.line(self.screen, BLACK, pko, p1o, width=int(100 * RSCALE))
        ...
        pygame.draw.circle(self.screen, GRAY , pko, 50/5 * np.sqrt(RSCALE))

        pygame.draw.line(self.screen, BLACK, [0,SCREEN_SIZE[1]/2 + offset_vert], [SCREEN_SIZE[0], SCREEN_SIZE[1]/2 + offset_vert])
        #text = font.render("t={:.02f} r={:.02f} ".format(t, energy, r), True, BLACK)
        text = font.render("t={:.02f} E={:.02f} ref-th0={:.02f} th0={:.02f} ".format(t, energy, np.rad2deg(ref[0]), np.rad2deg(state[mp.IDX_th0])), True, BLACK)
...
class RabbitEnv(gym.Env):
    ...
    def step_ref(self, v_ref):
        ...
        new_ref = ref.copy()
        new_ref = mp.ref_clip(ref + v_ref)

        # 0. pd control
        #u = mp.pdcontrol_r_th(s, new_ref)
        u = mp.pdcontrol_th(s, new_ref)
        u = mp.torq_limit(s, u)

        # 1. update model
        t1 = t + dt
        while t < t1:
            mode, t, s = mp.step(t, s, u, frateG*dt if s[mp.IDX_z] == 0 else frateA*dt)
            done = game_over(s)
            reward = 0
    ...
    def info(self, frame=-1):
        ...
        print(f"INPUT: ref: {ref[0]} {ref[1] * 180/np.pi} u: {u/mp.max_u()}")
        print(f"OUTPUT:")
        mp.print_state(s)
        print(f"--------------")
        ...
        for event in pygame.event.get():
            if event.type == pl.QUIT:
                ...
                # frame rate
                    ...
                    v = k_th0 * np.array([1, 0])
                elif keyname == 'k':
                    v = k_th0 * np.array([-1, 0])
                elif keyname == 'h':
                    v = k_th1 * np.array([0, 1])
                elif keyname == 'l':
                    v = k_th1 * np.array([0, -1])
