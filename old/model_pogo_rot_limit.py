...
ref_max_th1 = np.deg2rad(100)
ref_min_th1 = np.deg2rad(-20)

max_th0 = np.deg2rad(90)
min_th0 = np.deg2rad(5)
max_th1 = np.deg2rad(150)
min_th1 = np.deg2rad(-40)
...
#MAX_TORQUE0=400 # knee can keep 100kg weight at pi/2 + arm
...
def postprocess(s):
    ...
    new_s[IDX_th0] = max(s[IDX_th0], min_th0)
    return new_s
...
def node_vel(s):
    ...
    v2 = v1 + np.array([cos(thr+th1), -sin(thr+th1)]) * l1 * (dthr+dth1)
...
def ground_limitAb(s, u):
    ...
    taur = 0
    tau1 = u[1]
    extf = np.array([fz, taur, tau1]).reshape(3,1)
    ...
    A[0][2] = -l1*m2*sin(th1)
    A[1][0] = -l1*m2*sin(th1)
    A[1][1] = (m2+m1+m0)*z0**2+((2*m2+2*m1+2*m0)*z+2*l1*m2*cos(th1)+4*l0*m2+4*l0*m1)*z0+(m2+m1+m0)*z**2+(2*l1*m2*cos(th1)+4*l0*m2+4*l0*m1)*z+4*l0*l1*m2*cos(th1)+(l1**2+4*l0**2)*m2+4*l0**2*m1
    A[1][2] = l1*m2*cos(th1)*z0+l1*m2*cos(th1)*z+2*l0*l1*m2*cos(th1)+l1**2*m2
    A[2][0] = -l1*m2*sin(th1)
    A[2][1] = l1*m2*cos(th1)*z0+l1*m2*cos(th1)*z+2*l0*l1*m2*cos(th1)+l1**2*m2
    A[2][2] = l1**2*m2

    b = np.zeros((3,1), dtype=np.float64)
    b[0] = (-dthr**2*m2*sin(thr)**2*z0)-dthr**2*m1*sin(thr)**2*z0-dthr**2*m0*sin(thr)**2*z0-dthr**2*m2*cos(thr)**2*z0-dthr**2*m1*cos(thr)**2*z0-dthr**2*m0*cos(thr)**2*z0-dthr**2*m2*sin(thr)**2*z-dthr**2*m1*sin(thr)**2*z-dthr**2*m0*sin(thr)**2*z-dthr**2*m2*cos(thr)**2*z-dthr**2*m1*cos(thr)**2*z-dthr**2*m0*cos(thr)**2*z+k*z-dthr**2*l1*m2*sin(thr)*sin(thr+th1)-2*dth1*dthr*l1*m2*sin(thr)*sin(thr+th1)-dth1**2*l1*m2*sin(thr)*sin(thr+th1)-dthr**2*l1*m2*cos(thr)*cos(thr+th1)-2*dth1*dthr*l1*m2*cos(thr)*cos(thr+th1)-dth1**2*l1*m2*cos(thr)*cos(thr+th1)-2*dthr**2*l0*m2*sin(thr)**2-2*dthr**2*l0*m1*sin(thr)**2-2*dthr**2*l0*m2*cos(thr)**2-2*dthr**2*l0*m1*cos(thr)**2+g*m2*cos(thr)+g*m1*cos(thr)+g*m0*cos(thr)+c*dz
    b[1] = (-2*dth1*dthr*l1*m2*cos(thr)*sin(thr+th1)*z0)-dth1**2*l1*m2*cos(thr)*sin(thr+th1)*z0+2*dth1*dthr*l1*m2*sin(thr)*cos(thr+th1)*z0+dth1**2*l1*m2*sin(thr)*cos(thr+th1)*z0+2*dthr*dz*m2*sin(thr)**2*z0+2*dthr*dz*m1*sin(thr)**2*z0+2*dthr*dz*m0*sin(thr)**2*z0-g*m2*sin(thr)*z0-g*m1*sin(thr)*z0-g*m0*sin(thr)*z0+2*dthr*dz*m2*cos(thr)**2*z0+2*dthr*dz*m1*cos(thr)**2*z0+2*dthr*dz*m0*cos(thr)**2*z0-2*dth1*dthr*l1*m2*cos(thr)*sin(thr+th1)*z-dth1**2*l1*m2*cos(thr)*sin(thr+th1)*z+2*dth1*dthr*l1*m2*sin(thr)*cos(thr+th1)*z+dth1**2*l1*m2*sin(thr)*cos(thr+th1)*z+2*dthr*dz*m2*sin(thr)**2*z+2*dthr*dz*m1*sin(thr)**2*z+2*dthr*dz*m0*sin(thr)**2*z-g*m2*sin(thr)*z-g*m1*sin(thr)*z-g*m0*sin(thr)*z+2*dthr*dz*m2*cos(thr)**2*z+2*dthr*dz*m1*cos(thr)**2*z+2*dthr*dz*m0*cos(thr)**2*z+2*dthr*dz*l1*m2*sin(thr)*sin(thr+th1)-4*dth1*dthr*l0*l1*m2*cos(thr)*sin(thr+th1)-2*dth1**2*l0*l1*m2*cos(thr)*sin(thr+th1)-g*l1*m2*sin(thr+th1)+4*dth1*dthr*l0*l1*m2*sin(thr)*cos(thr+th1)+2*dth1**2*l0*l1*m2*sin(thr)*cos(thr+th1)+2*dthr*dz*l1*m2*cos(thr)*cos(thr+th1)+4*dthr*dz*l0*m2*sin(thr)**2+4*dthr*dz*l0*m1*sin(thr)**2-2*g*l0*m2*sin(thr)-2*g*l0*m1*sin(thr)+4*dthr*dz*l0*m2*cos(thr)**2+4*dthr*dz*l0*m1*cos(thr)**2
    b[2] = dthr**2*l1*m2*cos(thr)*sin(thr+th1)*z0-dthr**2*l1*m2*sin(thr)*cos(thr+th1)*z0+dthr**2*l1*m2*cos(thr)*sin(thr+th1)*z-dthr**2*l1*m2*sin(thr)*cos(thr+th1)*z+2*dthr*dz*l1*m2*sin(thr)*sin(thr+th1)+2*dthr**2*l0*l1*m2*cos(thr)*sin(thr+th1)-g*l1*m2*sin(thr+th1)-2*dthr**2*l0*l1*m2*sin(thr)*cos(thr+th1)+2*dthr*dz*l1*m2*cos(thr)*cos(thr+th1)
...
def air_limitAb(s, u):
    ...
    dth1 = s[IDX_dth1]
    tau1 = u[1]
    extf = np.array([0, 0, 0, tau1]).reshape(4,1)

    A = np.zeros((4,4), dtype=np.float64)
    A[0][0] = m2+m1+m0
    A[0][1] = 0
    A[0][2] = (m2+m1+m0)*cos(thr)*z0+l1*m2*cos(thr+th1)+(2*l0*m2+2*l0*m1)*cos(thr)
    A[0][3] = l1*m2*cos(thr+th1)
    A[1][0] = 0
    A[1][1] = m2+m1+m0
    A[1][2] = ((-m2)-m1-m0)*sin(thr)*z0-l1*m2*sin(thr+th1)+((-2*l0*m2)-2*l0*m1)*sin(thr)
    A[1][3] = -l1*m2*sin(thr+th1)
    A[2][0] = (m2+m1+m0)*cos(thr)*z0+l1*m2*cos(thr+th1)+(2*l0*m2+2*l0*m1)*cos(thr)
    A[2][1] = ((-m2)-m1-m0)*sin(thr)*z0-l1*m2*sin(thr+th1)+((-2*l0*m2)-2*l0*m1)*sin(thr)
    A[2][2] = (m2+m1+m0)*z0**2+(2*l1*m2*cos(th1)+4*l0*m2+4*l0*m1)*z0+4*l0*l1*m2*cos(th1)+(l1**2+4*l0**2)*m2+4*l0**2*m1
    A[2][3] = l1*m2*cos(th1)*z0+2*l0*l1*m2*cos(th1)+l1**2*m2
    A[3][0] = l1*m2*cos(thr+th1)
    A[3][1] = -l1*m2*sin(thr+th1)
    A[3][2] = l1*m2*cos(th1)*z0+2*l0*l1*m2*cos(th1)+l1**2*m2
    A[3][3] = l1**2*m2

    b = np.zeros((4,1), dtype=np.float64)
    b[0] = (-dthr**2*m2*sin(thr)*z0)-dthr**2*m1*sin(thr)*z0-dthr**2*m0*sin(thr)*z0-dthr**2*l1*m2*sin(thr+th1)-2*dth1*dthr*l1*m2*sin(thr+th1)-dth1**2*l1*m2*sin(thr+th1)-2*dthr**2*l0*m2*sin(thr)-2*dthr**2*l0*m1*sin(thr)
    b[1] = (-dthr**2*m2*cos(thr)*z0)-dthr**2*m1*cos(thr)*z0-dthr**2*m0*cos(thr)*z0-dthr**2*l1*m2*cos(thr+th1)-2*dth1*dthr*l1*m2*cos(thr+th1)-dth1**2*l1*m2*cos(thr+th1)-2*dthr**2*l0*m2*cos(thr)-2*dthr**2*l0*m1*cos(thr)+g*m2+g*m1+g*m0
    b[2] = (-2*dth1*dthr*l1*m2*cos(thr)*sin(thr+th1)*z0)-dth1**2*l1*m2*cos(thr)*sin(thr+th1)*z0+2*dth1*dthr*l1*m2*sin(thr)*cos(thr+th1)*z0+dth1**2*l1*m2*sin(thr)*cos(thr+th1)*z0-g*m2*sin(thr)*z0-g*m1*sin(thr)*z0-g*m0*sin(thr)*z0-4*dth1*dthr*l0*l1*m2*cos(thr)*sin(thr+th1)-2*dth1**2*l0*l1*m2*cos(thr)*sin(thr+th1)-g*l1*m2*sin(thr+th1)+4*dth1*dthr*l0*l1*m2*sin(thr)*cos(thr+th1)+2*dth1**2*l0*l1*m2*sin(thr)*cos(thr+th1)-2*g*l0*m2*sin(thr)-2*g*l0*m1*sin(thr)
    b[3] = dthr**2*l1*m2*cos(thr)*sin(thr+th1)*z0-dthr**2*l1*m2*sin(thr)*cos(thr+th1)*z0+2*dthr**2*l0*l1*m2*cos(thr)*sin(thr+th1)-g*l1*m2*sin(thr+th1)-2*dthr**2*l0*l1*m2*sin(thr)*cos(thr+th1)
...
def f_air_limit(t, s, u, params={}):
    A, b, extf = air_limitAb(s, u)
    assert np.linalg.matrix_rank(A) == 4, (s, A)
    #Ax + b = extf
    #print(np.linalg.det(A))
...
def f_ground_limit(t, s, u, params={}):
    A, b, extf = ground_limitAb(s, u)
    assert np.linalg.matrix_rank(A) == 3
    y = np.linalg.solve(A, extf-b).reshape(3)
...
def check_limit_event(s, ds, dt):
    s1 = s + ds * dt
    th0 = s[IDX_th0]
    th0_1 = s1[IDX_th0]
    if th0 > min_th0 and th0_1 > min_th0:
        return "no-limit", dt
    elif th0 > min_th0 and th0_1 <= min_th0:
        return "limit-on", (th0-min_th0) * dt / (th0 - th0_1)
    elif th0 <= min_th0 and th0_1 <= min_th0:
        return "limit", dt
    else:
        return "limit-off", dt
...
def impulse_limit_ground(s):
    ...
    A = np.zeros((3,3), dtype=np.float64)
    A[0][0] = (m2+m1+m0)*cos(thr)*z0+(m2+m1+m0)*cos(thr)*z-l1*m2*sin(th1)*sin(thr)+(l1*m2*cos(th1)+(2*l0*m2+2*l0*m1)*cos(th0))*cos(thr)
    A[0][1] = (m2+m1+m0)*sin(thr)
    A[0][2] = l1*m2*cos(th1)*cos(thr)-l1*m2*sin(th1)*sin(thr)
    A[1][0] = ((-m2)-m1-m0)*sin(thr)*z0+((-m2)-m1-m0)*sin(thr)*z+(((-2*l0*m2)-2*l0*m1)*cos(th0)-l1*m2*cos(th1))*sin(thr)-l1*m2*sin(th1)*cos(thr)
    A[1][1] = (m2+m1+m0)*cos(thr)
    A[1][2] = (-l1*m2*cos(th1)*sin(thr))-l1*m2*sin(th1)*cos(thr)
    A[2][0] = -((-4*l0*l1*m0*m2*cos(th0)*cos(th1))+((-4*l0**2*m0*m2)-4*l0**2*m0*m1)*cos(th0)**2+((-l1**2*m1)-l1**2*m0)*m2)/(m2+m1+m0)
    A[2][1] = 0
    A[2][2] = -(((-l1**2*m1)-l1**2*m0)*m2-2*l0*l1*m0*m2*cos(th0)*cos(th1))/(m2+m1+m0)

    b = np.zeros((3,1), dtype=np.float64)
    b[0] = ((-dthr*m2)-dthr*m1-dthr*m0)*cos(thr)*z0+((-dthr*m2)-dthr*m1-dthr*m0)*cos(thr)*z+((dthr+dth1)*l1*m2*sin(th1)+(2*dth0*l0*m2+2*dth0*l0*m1)*sin(th0)-dz*m2-dz*m1-dz*m0)*sin(thr)+(((-dthr)-dth1)*l1*m2*cos(th1)+((-2*dthr*l0*m2)-2*dthr*l0*m1)*cos(th0))*cos(thr)
    b[1] = (dthr*m2+dthr*m1+dthr*m0)*sin(thr)*z0+(dthr*m2+dthr*m1+dthr*m0)*sin(thr)*z+((dthr+dth1)*l1*m2*cos(th1)+(2*dthr*l0*m2+2*dthr*l0*m1)*cos(th0))*sin(thr)+((dthr+dth1)*l1*m2*sin(th1)+(2*dth0*l0*m2+2*dth0*l0*m1)*sin(th0)-dz*m2-dz*m1-dz*m0)*cos(thr)
    b[2] = -(2*dth0*l0*l1*m0*m2*sin(th0)*sin(th1)+(4*dthr+2*dth1)*l0*l1*m0*m2*cos(th0)*cos(th1)+(4*dthr*l0**2*m0*m2+4*dthr*l0**2*m0*m1)*cos(th0)**2+((dthr+dth1)*l1**2*m1+(dthr+dth1)*l1**2*m0)*m2)/(m2+m1+m0)

    #y:ratsimp([dthr', dz',dth1'])
    y = np.linalg.solve(A, -b).reshape(3)
    new_s[IDX_dthr] = y[0]
    new_s[IDX_dz] = y[1]
    new_s[IDX_dth1] = y[2]
...
def impulse_limit_air(s):
    ...
    A[0][0] = m2+m1+m0
    A[0][1] = 0
    A[0][2] = l1*m2*cos(thr+th1)
    A[1][0] = 0
    A[1][1] = m2+m1+m0
    A[1][2] = -l1*m2*sin(thr+th1)
    A[2][0] = 0
    A[2][1] = 0
    A[2][2] = ((l1**2*m1+l1**2*m0)*m2*sin(thr+th1)**2+2*l0*l1*m0*m2*cos(th0)*sin(thr)*sin(thr+th1)+(l1**2*m1+l1**2*m0)*m2*cos(thr+th1)**2+2*l0*l1*m0*m2*cos(th0)*cos(thr)*cos(thr+th1))/(m2+m1+m0)

    b = np.zeros((3,1), dtype=np.float64)
    b[0] = (-dth1*l1*m2*cos(thr+th1))+(2*dth0*l0*m2+2*dth0*l0*m1)*sin(th0)*sin(thr)-dxr*m2-dxr*m1-dxr*m0
    b[1] = dth1*l1*m2*sin(thr+th1)+(2*dth0*l0*m2+2*dth0*l0*m1)*sin(th0)*cos(thr)-dyr*m2-dyr*m1-dyr*m0
    b[2] = -((dth1*l1**2*m1+dth1*l1**2*m0)*m2*sin(thr+th1)**2+(2*dth1*l0*l1*m0*m2*cos(th0)*sin(thr)+2*dth0*l0*l1*m0*m2*sin(th0)*cos(thr))*sin(thr+th1)+(dth1*l1**2*m1+dth1*l1**2*m0)*m2*cos(thr+th1)**2+(2*dth1*l0*l1*m0*m2*cos(th0)*cos(thr)-2*dth0*l0*l1*m0*m2*sin(th0)*sin(thr))*cos(thr+th1))/(m2+m1+m0)
...
def step(t, s, u, dt):
    ...
    if s[IDX_yr] == 0 and s[IDX_dx] == 0 and s[IDX_dy] == 0: # already on the ground
        ...
        limev, limt = check_limit_event(s, ds, dt)
        mint = min(jmpt, limt)

        # limit and jump case is missed
        if jmpt == mint:
            ...
            ds = rk4(f_ground, t, s, u, {}, limt)
            s = s + ds * limt
            s = impulse_limit_ground(s)
            s = limit_th0(s)
            return  "limit-on", t + limt, postprocess(s)
        elif limev == "limit-off":
            return "limit-off", t + dt, postprocess(s + ds * dt)
        elif limev == "limit":
            ds = rk4(f_ground_limit, t, s, u, {}, dt)
            s = s + ds * dt
            s = limit_th0(s)
            return  "limit-ground", t + dt, postprocess(s)
        elif limev == "no-limit":
        ...
        if colt == mint:
            ...
            ds = rk4(f_air, t, s, u, {}, limt)
            s = s + ds * limt
            s = impulse_limit_air(s)
            s = limit_th0(s)
            return  "limit-on", t + limt, postprocess(s)
        elif limev == "limit-off":
            return "limit-off", t + dt, postprocess(s + ds * dt)
        elif limev == "limit":
            ds = rk4(f_air_limit, t, s, u, {}, dt)
            s = s + ds * dt
            s = limit_th0(s)
            return  "limit-air", t + dt, postprocess(s)
        elif limev == "no-limit":
...
def amoment(s):
    vr, v0, v1, v2 = node_vel(s)
    pr, p0, pk, p1, p2 = node_pos(s)
    return m0*cross(v0-vr, p0-pr) + m1*cross(v1-vr, p1-pr) + m2*cross(v2-vr, p2-pr)
...
def pdcontrol(s, ref):
    ...
    err = ref - ob
    return err * Kp - Kd * dob

# (d, th1) --PD--> u
def pdcontrol_inv(s, ref):
    dob  = np.array([s[IDX_dth0], s[IDX_dth1]])
    ob = np.array([s[IDX_th0], s[IDX_th1]])
    err = ref - ob
    ddth = err * Kp - Kd * dob
